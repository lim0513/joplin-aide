...
An AI assistant chat panel for [Joplin](https://joplinapp.org/) — ask about your notes and let the AI read, search, create and edit them, powered by the CLI you already have ([Claude Code](https://claude.com/claude-code), [GitHub Copilot CLI](https://github.com/features/copilot/cli), [OpenAI Codex CLI](https://developers.openai.com/codex/cli) or [Google Antigravity CLI](https://antigravity.google/product/antigravity-cli)) or the [Kimi (Moonshot)](https://platform.moonshot.cn/) API — no CLI required.
...
- At least one backend CLI, installed and logged in:
  ...
  - [Google Antigravity CLI](https://antigravity.google/docs/cli/install/) — `agy` on PATH or the Windows installer location (found automatically); run `agy` once in a terminal to sign in. Headless agy cannot ask for permission, so the plugin keeps its allow rules (`mcp(joplin/*)` plus the "additional allowed tools" setting, default `read_url(*)`) in `~/.gemini/antigravity-cli/settings.json`; rules you add there yourself are left alone
